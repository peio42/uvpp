...
| Signal, timer, idle, prepare, check, async, poll, TTY | historical handle wrappers in `uv` | `uv::raw` until a distinct high-level owner exists | Not migrated | Do not treat the historical wrapper API as the future owner contract. |
