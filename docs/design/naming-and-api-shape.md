...
| `spawn_handle<T>` owns a root frame, exposes multi-observer same-loop `join()`, cooperative `request_stop()`, and a single-consumer non-void `take_result()`. | Keep the name provisionally. Active destruction still terminates as an implementation-stage diagnostic, not the target. Post-destruction retention, stop, reclamation, and unobserved-error routing remain implementation work in [003](../proposals/003-cancellation-and-task-scopes.md). |
