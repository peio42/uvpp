...
| `spawn_handle<T>` observes a root frame, exposes multi-observer same-loop `join()`, cooperative `request_stop()`, and a single-consumer non-void `take_result()`. | Keep the name provisionally. Active destruction requests stop while an internal baton retains and safely reclaims the root. General unobserved-error routing remains implementation work in [003](../proposals/003-cancellation-and-task-scopes.md). |
