...
| TCP/pipe connections have internal close completion but no public `close()`. Listeners and UDP have synchronous `void close()` that releases the owner and initiates deferred cleanup. | Settled for v3: owner `close()` is awaitable and completes only after native close; `request_close()` explicitly initiates close without awaiting. The contract, including active-operation rules, remains in [002](../proposals/002-async-ownership.md). This is a target API, not a cosmetic rename of the experimental methods. |
